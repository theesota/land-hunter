...
// キー名を落としてサイズを約半分にする
function toTuple(s) {
  return [s.id, s.lat, s.lng, s.name, s.status, s.rating, s.memo, s.createdAt, s.updatedAt, s.url || '', s.info || null, s.roads || []];
}

function fromTuple(t) {
  if (!Array.isArray(t) || t.length < 9) return null;
  const [id, lat, lng, name, status, rating, memo, createdAt, updatedAt, url, info, roads] = t;
  const s = {
